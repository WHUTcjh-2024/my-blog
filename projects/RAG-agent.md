...
::: tip 面试时的一句话
我把大模型放在“意图理解、工具选择和推荐解释”这一层，商品召回、参数校验和状态变更全部由确定性服务完成，因此 LLM 不可用时核心流程仍能运行，也不能生成目录外商品。
:::
...
> Atelier 是一个基于真实 H&M 商品目录的多模态导购 Agent。文本侧使用多语言 MiniLM，图片侧使用 CLIP，再融合结构化条件和交易热度。Agent 将检索、对比、购物车和结算封装成白名单工具；LLM 只负责工具选择和候选集内的推荐理由，商品 ID 经过 Pydantic 与服务端白名单双重校验。系统还实现了 SQLite 多轮记忆、SSE 流式返回、无 LLM 回退、Docker 和自动化测试。当前评测主要是 5,000 商品上的回归用例，我不会把它描述成百万级生产系统。
