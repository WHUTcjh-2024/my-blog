...
这里贴出运行结果：
![线性回归实验结果](/images/deep-learning/image.png)
...
上面完成了线性回归的训练闭环。它既是回归任务的基础模型，也是检查数据、损失函数、优化器与评估流程是否正确的实用基线。

## 工程价值

1. **解决什么问题：** 线性回归提供连续值预测基线，也用于验证数据管道、损失计算和优化器是否正确工作。
2. **适合场景：** 特征与目标近似线性、需要强可解释性，或需要快速建立可比较的基线。
3. **不适合场景：** 强非线性关系、复杂图像与文本输入，或需要自动学习高阶交互的任务。
4. **PyTorch 常见坑：** 标签与输出 shape 不一致触发广播、未标准化导致优化困难，以及把训练损失下降误当成验证性能提升。
5. **项目联系：** SIREN/PINNs 项目同样依赖“输入—预测—损失—反向传播—验证”的闭环，区别在于模型表达能力和物理损失更复杂。
